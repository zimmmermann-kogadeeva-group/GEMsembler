...
class Selected(object):
    def __init__(self, highest: list, consistent: list):
...
def checkDBConsistency(
    ...
    if useroutname is not None:
        filename_changed = (
            "../Output/" + useroutname + "_" + obj_type + "_changed_for_consistency.tsv"
        )
        filename_notconsistent = (
            "../Output/" + useroutname + "_" + obj_type + "_not_consistent.tsv"
        )
    else:
    ...
    for bd, models in models_same_db.items():
                    ...
                    if len(common_ids) > 0:
                        for present in types_present:
                            if sorted(common_ids) != sorted(
                                        ...
                                        pd.DataFrame(
                                            [
                                                [
                                                    ...
                                                    id,
                                                    common_ids,
                                                    highest_converted.get(present).get(
                                                        id
                                                    )[1],
                                                ]
                                            ],
                                ...
                                changed_stat.get(present)["total_change"] = (
                                    changed_stat.get(present)["total_change"] + 1
                                )
                                if (len(common_ids) == 1) & (
                                    len(highest_converted.get(present).get(id)[1]) > 1
                                ):
                                    changed_stat.get(present)["n_to_one"] = (
                                        changed_stat.get(present)["n_to_one"] + 1
                                    )
                                elif (len(common_ids) == 1) & (
                                    len(highest_converted.get(present).get(id)[1]) == 0
                                ):
                                    changed_stat.get(present)["zero_to_one"] = (
                                        changed_stat.get(present)["zero_to_one"] + 1
                                    )
                                elif (len(common_ids) > 1) & (
                                    len(highest_converted.get(present).get(id)[1]) > 1
                                ):
                                    changed_stat.get(present)["n_to_n"] = (
                                        changed_stat.get(present)["n_to_n"] + 1
                                    )
                                elif (len(common_ids) > 1) & (
                                    len(highest_converted.get(present).get(id)[1]) == 0
                                ):
                                    changed_stat.get(present)["zero_to_n"] = (
                                        changed_stat.get(present)["zero_to_n"] + 1
                                    )
                            else:
                                consistent_highest.get(present).update(
                                    {
                        ...
                        for pr in types_present:
                            not_consistent_highest.get(pr).update(
                                {
                                    id: [
                                        highest_converted.get(pr).get(id)[0],
                                        highest_converted.get(pr).get(id)[1],
                                    ]
                                }
                            )
                            none_consistent_highest = pd.concat(
                                [
                                    ...
                                    pd.DataFrame(
                                        ...
                                        columns=none_consistent_highest.columns,
                                    ),
                                ],
                                ignore_index=True,
                            )
        ...
        if do_stat:
            for key, value in changed_stat.items():  # TODO: write this stat in file
                print(
                    f"For {key} model in total {value['total_change']} {obj_type} were changed"
                )
                print(
                    f"For {key} model {value['n_to_one']} {obj_type} were changed from several ids to one"
                )
                print(
                    f"For {key} model {value['zero_to_one']} {obj_type} were changed from no found ids to one"
                )
                print(
                    f"For {key} model {value['n_to_n']} {obj_type} were changed from several ids to smaller amount of ids"
                )
                print(
                    f"For {key} model {value['zero_to_n']} {obj_type} were changed from no found ids to several"
                )
...
def checkFromOneFromMany(model_types: [str], to_smth: dict):
    ...
    for typ in model_types:
        to_smth_str = {
            orig_id: [bigg_ids[0], " ".join(sorted(bigg_ids[1]))]
            for orig_id, bigg_ids in to_smth.get(typ).items()
        }
        reversed_to_smth = {bigg_id[1]: [] for bigg_id in to_smth_str.values()}
        for (k, v) in to_smth_str.items():
            reversed_to_smth[v[1]].append(k)
        from_one.update(
            {
                typ: {
                    orig_ids[0]: [
                        to_smth.get(typ).get(orig_ids[0])[0],
                        conv_id.split(" "),
                    ]
                    for conv_id, orig_ids in reversed_to_smth.items()
                    if len(orig_ids) == 1
                }
            }
        )
...
def runSelection(
    ...
    consistent, consist, not_consistent = checkDBConsistency(
        models_same_db, highest, obj_type
    )
